...
function afegirCampsTasca(divTasca, nom = '', descripcio = '', dataInici = '', dataFi = '', encarregat = '', estat = 'Pendent') {
    ...
    function carregarEncarregats() {
        fetch('/php/selectColaboradorsProjecte.php')
            .then(response => response.json())
            .then(encarregats => {
                seleccionarEncarregat.innerHTML = '';
                
                const opcioPrincipal = document.createElement('option');
                opcioPrincipal.textContent = 'Seleccionar encarregat';
                opcioPrincipal.disabled = true;
                opcioPrincipal.selected = true;
                seleccionarEncarregat.appendChild(opcioPrincipal);
